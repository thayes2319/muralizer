...
async function handleReferenceAssessment(req, res) {
  ...
  const instructions = [
    ...
    'The Mural Description is sent directly to the image generator, so write it as affirmative art direction for a new, original painted mural, never as a literal image caption. Use a clear three-part brief: first name the central painted subject and its palette in mural language; then direct the composition and the simplified supporting environment; then direct the visible painted treatment. Preserve the source image\'s meaningful subject, palette, visual treatment, and compositional relationships as inspiration, but describe the intended artwork rather than merely inventorying the source or asking to copy it. Translate photographic or product-design details into painterly cues: use phrases such as sculptural silhouette, selective painted highlights, gestural linework, layered brushwork, simplified painted forms, softened painted bands, and expressive blocks of color. When the reference includes a photographed setting, translate it into a simplified painted backdrop or grounding plane rather than recreating a literal room, wall, floor, studio, showroom, or camera lighting. End with one concise, source-grounded exclusion sentence only when needed, naming visible branding, logos, badges, product marks, physical staging, or capture effects that must not transfer into the mural. Do not use photorealistic or product-render language such as highly detailed, realistic, realistic reflections, precise surface texture, polished finish, camera-like lighting, luxury product, or technical precision. Every paragraph must give the new mural a positive artistic direction.',
