...
async function handleReferenceAssessment(req, res) {
  ...
  const instructions = [
    ...
    'The Mural Description is sent directly to the image generator, so write three short paragraphs of affirmative art direction for a new, original painted mural, never a literal image caption. Paragraph 1 must begin "A hand-painted mural of" and give the depicted subject, a shared painted palette, and a concrete simplified setting from the reference as connected elements of one composition. Paragraph 2 must begin "Keep" and direct the overall composition, pose, scale, and meaningful spatial relationships; it must then begin "Let" to tell the setting how to resolve into simplified painted forms that share the subject\'s brushwork and visual plane. Paragraph 3 must begin "Use" and name visible painted techniques that translate any material or technical cues in the reference, then require a flat, expressive decorative-mural character. Preserve the source image\'s meaningful subject, palette, visual treatment, and compositional relationships as inspiration, but describe the intended artwork rather than merely inventorying the source or asking to copy it. Translate photographic or product-design details into painterly cues: use phrases such as sculptural silhouette, selective painted highlights, gestural linework, layered brushwork, simplified painted forms, softened painted bands, and expressive blocks of color. Do not use vague promotional or product-render phrasing such as bold statement, mechanical elegance, dynamic form, reflective surfaces, aerodynamic details, highly detailed, realistic, realistic reflections, precise surface texture, polished finish, camera-like lighting, luxury product, or technical precision. When the reference includes a photographed setting, translate it into simplified painted forms that share the subject\'s visual plane rather than recreating a literal room, wall, floor, studio, showroom, or camera lighting. Every paragraph must give the new mural positive, concrete artistic direction.',
