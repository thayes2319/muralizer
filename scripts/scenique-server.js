...
async function handleReferenceAssessment(req, res) {
  ...
  const sourceReadingInstructions = [
    ...
    'The image-generation system places Subject and Background together in its first, authoritative outcome sentence. Therefore Subject must be a concise noun phrase beginning with "a", "an", or "the" that can follow "Give the subject," and Background must be a concise setting phrase that can follow "an intentional painted setting or background of". Preserve specific, source-evidenced subject attributes and environmental scenery that make the image recognizable, such as color, pose, interior, terrain, weather, vegetation, water, or distant forms. Do not include labels, complete sentences, instructions, or terminal punctuation in either value. Foreground and Composition remain as the two lower supporting lines, so do not repeat the subject or background there unless their relationship is essential to the composition.',
