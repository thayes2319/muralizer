...
async function handleReferenceGenerateProxy(req, res) {
  ...
  // results either way.
  const paintedStyle = 'Render this as a clearly non-photographic, hand-painted or hand-illustrated mural artwork with visible brushwork, simplified painted forms, artistic texture, and hand-rendered color blending. Treat the reference image strictly as a guide to subject placement and composition, never as a rendering exemplar. Reinterpret words such as realistic, reflective, metallic, polished, detailed, or precise as selective painted cues, not physically accurate materials, camera-like lighting, glossy surfaces, or photographic surface detail. Abstract and simplify all natural elements such as foliage, water, and light into visible brushstrokes, not photographic detail. Ignore any glare, reflections, or lighting artifacts from the reference photo being captured under real light; those are not part of the artwork.';
...
async function handleReferenceAssessment(req, res) {
  ...
  const instructions = [
    ...
    'Use the visual evidence to write positive art direction for a new, original mural: preserve the source image\'s meaningful subject, palette, visual treatment, and compositional relationships as inspiration, but describe the intended artwork rather than merely inventorying the source or asking to copy it.',
