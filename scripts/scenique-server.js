...
const MURAL_SYNTHESIS_STYLE = 'STYLE REQUIREMENT -- takes priority over every realism cue in the mural brief: render a flat, expressive, clearly hand-painted decorative mural, with unmistakable broad brush marks, layered opaque color, simplified painted shapes, and deliberate hand-drawn linework. The result must read immediately as a two-dimensional artwork on a wall, never as a photograph, product render, or cinematic still. Reinterpret words such as realistic, reflective, metallic, polished, detailed, or precise as loose painted suggestions only: all shine, gleam, reflections, gloss, and metallic cues must be softened and toned down into sparse, broken painted linework and brushwork only, never continuous gradients or literal surface rendering. Never use physically accurate materials, glossy rendering, precise surface texture, camera-like lighting, or illusionistic depth. Convert depicted light, atmosphere, natural forms, built forms, and material surfaces into visible painterly marks and expressive blocks of color.';

function withMuralSynthesis(prompt) {
  return `${prompt}\n\n${MURAL_SYNTHESIS_STYLE}`;
}
...
async function handleGenerateProxy(req, res) {
  ...
  if (useExternalUpstream) {
    const response = await fetch(generateUpstream, {
      ...
      body: JSON.stringify({ ...body, prompt: withMuralSynthesis(String(body.prompt || '').trim()) })
    });
  ...
  const prompt = withMuralSynthesis(String(body.prompt || '').trim());
...
async function handleReferenceGenerateProxy(req, res) {
  ...
  form.append('prompt', `${withMuralSynthesis(prompt)}\n\n${muralOnly} Treat the reference image strictly as a guide to subject placement and composition, never as a rendering exemplar. Ignore any glare, reflections, or lighting artifacts from the reference photo being captured under real light; those are not part of the artwork. ${exclusions}`);
  form.append('output_format', 'png');
...
async function handleReferenceAssessment(req, res) {
  ...
  const instructions = [
    ...
    'The Mural Description is sent directly to the image generator, so write three short paragraphs of affirmative art direction for a new, original painted mural, never a literal image caption. Paragraph 1 must begin "A hand-painted mural of" and give the central subject, a specific painted palette, and a concrete simplified surrounding setting drawn from the reference. Paragraph 2 must begin "Keep" and direct the central motif\'s composition, pose, scale, and meaningful relationships; it must then begin "Let" to tell the supporting setting how to resolve into simplified painted forms that support rather than compete with the subject. Paragraph 3 must begin "Use" and name visible painted techniques that translate any material or technical cues in the reference, then require a flat, expressive decorative-mural character. Whenever the source depicts a real physical object or material, direct paint to only suggest it through selective painted highlights, layered brushwork, and gestural linework; do not direct literal material rendering, reflective surface behavior, precise curvature, or realistic object detailing. Preserve the source image\'s meaningful subject, palette, visual treatment, and compositional relationships as inspiration, but describe the intended artwork rather than merely inventorying the source or asking to copy it. Translate photographic or product-design details into painterly cues: use phrases such as sculptural silhouette, selective painted highlights, gestural linework, layered brushwork, simplified painted forms, softened painted bands, and expressive blocks of color. Never describe a subject through performance, status, engineering, material behavior, or technical precision; instead describe its visible silhouette, palette, pose, and painted marks. Do not use vague promotional or product-render phrasing such as high-performance, luxury, bold statement, mechanical elegance, dynamic form, reflective surfaces, metallic surfaces, aerodynamic details, complex mechanical details, sleekness, precision, technical sophistication, highly detailed, realistic, realistic reflections, precise surface texture, polished finish, camera-like lighting, or technical precision. When the reference includes a photographed setting, translate it into a simplified painted backdrop or grounding plane rather than recreating a literal room, wall, floor, studio, showroom, or camera lighting. Every paragraph must give the new mural positive, concrete artistic direction.',
