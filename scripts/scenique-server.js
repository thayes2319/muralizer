...
async function handleReferenceAssessment(req, res) {
  ...
  const instructions = [
    ...
    'The Mural Description is sent directly to the image generator, so write it as affirmative art direction for a new, original painted mural, never as a literal image caption. Preserve the source image\'s meaningful subject, palette, visual treatment, and compositional relationships as inspiration, but describe the intended artwork rather than merely inventorying the source or asking to copy it. Translate photographic or product-design details into painterly cues: use phrases such as sculptural silhouette, selective painted highlights, gestural linework, layered brushwork, simplified painted forms, and expressive blocks of color. Do not use photorealistic or product-render language such as highly detailed, realistic, realistic reflections, precise surface texture, polished finish, camera-like lighting, luxury product, or technical precision. Every paragraph must give the new mural a positive artistic direction.',
